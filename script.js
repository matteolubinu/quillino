...
(function () {
  ...
  // Auto-avanzamento: scorre le card da sola ogni 7s, in pausa su hover/
...
// ── Consenso cookie analytics (Google Analytics via Consent Mode v2) ───────
// Il default è 'denied' (impostato in <head>). Qui mostriamo il banner solo se
// l'utente non ha ancora scelto, e su "Accetta" sblocchiamo analytics_storage.
(function () {
  const KEY = 'quillino-analytics-consent';
  ...
  document.getElementById('cookieAccept').addEventListener('click', () => decide('granted'));
  document.getElementById('cookieReject').addEventListener('click', () => decide('denied'));
})();
